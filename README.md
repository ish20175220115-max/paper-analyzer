...
| 功能 | 决策 | 理由 |
|------|------|------|
| PDF 直接解析 | ❌ MVP 不做 | Markdown 输入门槛本身就是一道用户筛选——愿意整理 md 的用户才是核心用户 |
| 多轮对话 | ❌ V2 再做 | 先验证单篇分析质量；对话体验依赖知识库积累，冷启动阶段无意义 |
| 文献综述自动生成 | ❌ V3 再做 | 使用频率极低（每学期几次），过早做会分散核心验证精力 |
| 自动学习用户习惯 | ❌ 不做 | 听起来很 AI 但需要大量数据，MVP 阶段用固定模板 |
| 用户系统/协作 | ❌ 不做 | 单人单机，验证核心价值 |

### AI 产品 Badcase 分层框架

AI 产品的质量风险不是传统 bug，而是输出不可靠。本项目的 Badcase 框架分四层：

| 层级 | 类型 | 严重度 | 示例 |
|------|------|:--:|------|
| **L1** | 事实性错误 | 🔴 致命 | 编造不存在的论文结论、错误归因 |
| **L2** | 理解性偏差 | 🟠 严重 | 把"相关性"说成"因果性" |
| **L3** | 完整性缺失 | 🟡 中等 | 遗漏论文重要贡献点 |
| **L4** | 风格性不匹配 | 🟢 轻微 | 分析方向对但表达不精准 |
